...
255 bits could mathematically fit in 4 64-bit limbs, but in practice, representation schemes typically use 5 or more limbs.
...
Most software is checked for correctness by testing on known input-output pairs. Formal verification instead establishes correctness though machine-checked mathematical proofs that software matches a precise logical specification. 
...
We need to define these new atomic operations, for example \texttt{vadd} and \texttt{vsubb}:
...
\noindent to handle nested slices coming out of the \texttt{slice_vadd} rule.
...
\depgraph[.48]{SymexVectorBinopR}{Dependency graph of correspondence lemmas relating the symbolic and concrete semantics of lane-wise binary vector operations. Each lemma relates one layer of abstraction from each side.}
...
While correctness is the primary goal of this work, we also evaluate performance of our handwritten arithmetic primitives to demonstrate the performance gain from even simple vector optimizations.
...
Limb-parallel vector operations get no meaningful speedup. Field elements have five 64-bit limbs, but XMM and YMM registers store two and four limbs respectively, so the vectorized program still handles a tail in scalar instructions and pays overhead at the boundary. Limb-parallel optimizaiton might be more effective in representations with different limb counts.
