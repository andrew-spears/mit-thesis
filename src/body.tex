...
Fiat Cryptography and CryptOpt are recent projects that attempt to solve this problem by synthesizing low-level programs and guaranteeing correctness through formal verification. 
...
However, both Fiat Cryptography and CryptOpt are currently limited to scalar x86-64 instructions. Vectorized instructions perform operations in parallel and are increasingly relied upon in the most performant implementations, but these synthesis pipelines are incompatible with vectorized instruction sets, putting a hard ceiling on their performance capabilities.
...
Instead, they are represented using an array of 64-bit integers called \emph{limbs}.

One commonly used field is integers modulo $2^{255}-19$, whose largest element occupies 255 bits. 
This could mathematically fit in 4 64-bit limbs, but in practice, representation schemes typically use 5 or more limbs.
For example, the \question{ \emph{unsaturated Solinas}}{cite?} representation uses 5 64-bit limbs with headroom in each limb so that many operations can be applied independently to each limb before carrying between them. 
...
\begin{itemize}
    \item \emph{Limb-parallel} programs use vector registers to process multiple limbs of a single field element simultaneously --- for example, packing limbs 0 and 1 into one XMM register and adding them in a single \texttt{vpaddq}. These can be checked against existing specifications in Fiat Cryptography, since they are optimizations of scalar field operations.
...
Throughout this thesis we show Rocq code snippets: \question{\texttt{Definition} and \texttt{Fixpoint} introduce functions, \texttt{Lemma} states a theorem, and \texttt{Proof ... Qed} provides its proof.}
{not sure exactly what to include here. paper probably wont make any sense to someone who doesnt know any rocq anyway?}
...
\begin{enumerate}
    ...
    \item The specification is \textit{reified} --- converted from a Rocq function into an explicit syntax tree in PHOAS (Parametric Higher-Order Abstract Syntax) that can be inspected and transformed programmatically.
    ...
    \item The optimized expression is stringified into C code (or other target languages).
\end{enumerate}
...
The equivalence checker exists to verify the untrusted assembly programs output by CryptOpt\footnote{or another optimization process, including handwritten code. The equivalence checker resides in the Fiat-Crypto repository.}. At a high level, it does this by simulating the program and building up expressions representing each relevant value as a function of inputs (we call this \emph{symbolic execution}). This process is represented by a graph-based data structure called an \emph{E-graph}\footnote{A simplified version of an E-graph.}. Details are explained in \autoref{ch:architecture}.
...
The equivalence checker addresses this with a simple mapping called \emph{symbolic state} that maps each register and memory address to a previously created logical variable in the E-graph. 

After each instruction is symbolically executed, the equivalence checker runs a set of \emph{rewrite rules} that simplify the DAG by pattern-matching on node structure and replacing subexpressions with equivalent simpler forms.
These rewrites are essential for recognizing equivalence between two programs. Without this step, checking equivalence would be computationally expensive or undecidable.

By the end of symbolic execution, the logical variables associated with output registers and memory locations represent the program's outputs as expressions over the inputs, and equivalence between the program and specification \question{reduces to checking that outputs map to correct logical variables in symbolic state.}{Not sure if this wording make sense}

To symbolically execute an assembly program in this way, the equivalence checker needs a reference for how to execute each instruction, but also assurance that this symbolic execution is faithful to actual x86-64 semantics.
It achieves both by having two separate \question{descriptions}{Implementations? interpretations?} of each instruction: the semantic description, which encodes exactly what the instruction does on the machine; and the symbolic description, which describes how the instruction affects the DAG and symbolic state.
A layer in between then proves equivalence of the two descriptions.

This split achieves three things:
\begin{enumerate}
...
\question{Several projects}{Ive done my best to get this right and fact checked with an LLM, but I could be misunderstanding some of these and how they differ from fiat cryptography} address verified cryptographic implementations at different levels of abstraction.
...
Unlike our work, Jasmin verifies its own compiler's output rather than externally produced assembly, and currently targets only scalar x86-64 instructions.

\textbf{HACL*} is a verified cryptography library written in F*. It provides pure C implementations of cryptographic primitives that are formally verified for memory safety, functional correctness, and secret independence. The resulting code is portable but not optimized for platform-specific assembly.
...
However, HACL$\times$N operates at a higher level of abstraction: SIMD parallelism is expressed through F*'s type system with explicit vector types, avoiding reasoning about x86 semantics entirely. Assembly programs can be compiled from this point, but will not be provably correct.
...
Specifically, it uses a hash-consing DAG, where each node stores an operator and a list of child indices; when a new expression is inserted, its operands are canonicalized (e.g., sorted for commutative operators) and the DAG is scanned for an existing match.
If one is found, the existing index is returned rather than creating a duplicate node.
...
To \question{handle state,}{is this too confusing without a diagram?} a \textit{symbolic state} maps each register and memory address to a logical variable, which is itself a pointer into the DAG. Each new instruction looks up the logical variables for its source operands, builds an expression representing its effect, and writes a new logical variable back into the symbolic state.

In Rocq, these stateful operations are threaded through a symbolic state monad \texttt{M}, where the bind operator \bind{} sequences steps that read or update the state.
...
AVX includes 128-bit xmm and 256-bit ymm registers, and instructions that load and store 128 or 256 bits at a time.
Before implementing any vector instruction semantics, we needed to fully refactor the register and memory infrastructure to accommodate these wider operands.
...
We made the two register kinds distinguishable by type, with a unifying type REG = SReg SREG | VReg VREG.
...
Many existing functions for interacting with register values in Fiat-Crypto were specialized to handling 64-bit values.
...
Memory in Fiat-Crypto is represented in two ways.
...
This consistency is important because there is no overflow from one memory write to the next address -- writing to address a cannot influence address a+8 in this model, so the values need to be a consistent size.
...
For example, \mintinline{asm}{vmovdqu ymm0, [rbx]} loads 32 bytes of memory into the ymm0 register.
...
There are two principled ways to do this for any (nontrivial) instruction, each of which abstracts details to one place or another.
...
This means we need to define new symbolic operations, for example \texttt{vadd} and \texttt{vsubb}:
...
We were able to make this work for simple programs, but only by defining many ad-hoc, complex rewrite rules, which we explain in \autoref{sec:rewriting}. Based on this initial attempt, we decided that this approach was less principled, more complicated, and less scalable than the alternative.
...
\depgraph[.5]{VectorBinop both}{Dependency graph of functions in Symbolic.v and Semantics.v to implement vector symbolic execution and semantics. }
...
This makes the symbolic execution of most instructions simple to implement.
...
Instructions like \texttt{vpbroadcastq} (replicate one lane to all lanes), \texttt{vpblendd} (select lanes from two sources by bitmask), and \texttt{vpmuludq} (multiply lower 32 bits of each 64-bit lane) all admit a similar decomposition: slice the relevant parts of each source, apply a per-lane operation, and write results back via \texttt{set\_slice} in some order.
...
Recognizing that this is equivalent to a plain scalar addition \texttt{v1 + v2} requires the rewrite system to look three layers deep past the outermost slice.
...
Notice this ended up requiring the \texttt{coalescing_slice} helper function:
...
to handle nested slices coming out of the \texttt{slice_vadd} rule.
...
We were able to verify simple vector programs with these rules, but each new instruction pattern required additional ad-hoc rules, confirming that this approach would not scale.
...
These proofs are mostly short and mechanical for rules that only inspect one layer of the DAG. For rules like \texttt{slice\_set\_slice\_disjoint} which peel off layers recursively, we use inductive proofs.
...
However, there is no principled reason that batched specifications cannot be automatically derived from scalar versions. A Rocq function could generate a batched specification from any scalar specification by slicing inputs into groups and concatenating outputs, but we did not attempt this.
...
Because these two functions decompose with the same recursive structure, we can simply prove correspondence between horizontal layers in \autoref{fig:VectorBinop both}, as \question{diagrammed}{Im working on generating one with larger font} in \autoref{fig:SymexVectorBinopR}.

\depgraph[.4]{SymexVectorBinopR}{Dependency graph of correspondence lemmas relating the symbolic and concrete semantics of lane-wise binary vector operations. Each lemma relates one layer of abstraction from each side.}
...
Along with the main structural lemmas, these facts gives us all the building blocks needed to prove correspondence for all binary instructions. In fact, these proofs all collapse to a single line:
...
which applies the top-level correspondence lemma \texttt{SymexVectorBinop_R}, then clears the preconditions using a hint database of all the \texttt{interprets_as_[op]} lemmas.
...
Not all instructions operate independently and identically on each lane of source and destination values. \texttt{vpbroadcastq} broadcasts one lane to other lanes of a register; \texttt{vpblendd} picks lanes from two source registers based on a bitmask; \texttt{vpmuludq} multiplies the lower 32 bits of each lane, operating independently across lanes but requiring an extra slicing step that doesn't fit neatly into our previous abstraction.

While these instructions don't fit the same binop pattern, they can all be decomposed into repeated lane-wise steps. Mirroring this structure, we used recursive functions to build up each lane of the output for all such instructions, and continued the principle of proving correspondence after each step.

As an illustrative example, the blend fixpoint does one step of multiplexing between source registers for a single lane, then recursively blends the remaining lanes:
...
Following the same strategy as for binops, we prove a correspondence lemma for \texttt{blend_aux} by induction on the number of remaining lanes. Each inductive step relates one lane's slice-and-insert to its semantic counterpart.
...
A few instructions don't decompose into repeated lane-wise steps at all. \texttt{vpextrq}, \texttt{vextracti128}, and \texttt{vinserti128} each extract or insert a single fixed-width chunk at a constant offset, so their symbolic execution is just a single \texttt{slice} or \texttt{set_slice} operation. 
...
Working on such a large project necessitates proofs and tactics which are modular, intuitive, and robust to small changes in definitions or other tactics.
...
This has the added benefit that lemmas involving such expressions have a standardized way of stating them, so no extra transformation is needed to apply these lemmas.
...
This is theoretically simple because $\prec$ is transitive and reflexive, but not simple enough for Rocq to independently solve.
One way to solve this is using an all-purpose search like \texttt{eauto}, which works but is extremely opaque and can have downstream effects if it is called in the wrong goal state by accident.
A tactic which surgically matches on the goal and applies the appropriate transformations avoids both issues:
...
These principles made proofs for vector instructions extremely modular and interpretable, and we attempted to refactor many existing proofs similarly. This was possible for many helper lemmas, but not for the main correspondence proof of scalar instructions, which were far too sensitive to any changes and would require far more dedicated effort. As a \question{practical middle-ground}{Trying to explain that this was a difficult tradeoff, and future work could consolidate the two, but isolation is an ok solution since this is all proofs and just needs to compile}, the two instruction sets share many lower-level lemmas but use completely separate, self-contained sets of tactics at the higher level. 
...
With the extensions described in the preceding chapters, it now handles AVX2 vector instructions through the full pipeline: parsing, symbolic execution, rewriting, and equivalence checking.
...
Our test suite, listed in \autoref{tab:results}, covers both limb-parallel and batched SIMD programs, as described in \autoref{sec:avx-background}.

\begin{table}[h]
\centering
\begin{tabular}{lll}
\toprule
\textbf{Test name} & \textbf{Category} & \textbf{Operation} \\
\midrule
avx-xmm-add        & limb-parallel (xmm) & add \\
avx-ymm-add        & limb-parallel (ymm) & add \\
avx-xmm-sub        & limb-parallel (xmm) & sub \\
avx-ymm-sub        & limb-parallel (ymm) & sub \\
batch-avx-add       & batch & batch\_add \\
batch-avx-sub       & batch & batch\_sub \\
batch-avx-opp       & batch & batch\_opp \\
batch-avx-carry     & batch & batch\_carry \\
batch-avx-carry-add & batch & batch\_carry\_add \\
batch-avx-carry-sub & batch & batch\_carry\_sub \\
batch-avx-carry-opp & batch & batch\_carry\_opp \\
\bottomrule
\end{tabular}
\caption{Equivalence checking test suite for AVX2 assembly programs against Fiat Cryptography specifications. All programs target Curve25519 with 64-bit limbs.}
\label{tab:results}
\end{table}

Two representative example programs are shown in \autoref{fig:simple-add} and \autoref{fig:batch-add}.
...
%     ret
% #END
\codeshotfig{simple-add}{XMM vectorized field addition for Curve25519. Adds five 64-bit limbs using 128-bit \texttt{vpaddq} instructions, processing two limbs per iteration with a single-limb tail.}
...
%     ret
% #END
\codeshotfig{batch-add}{Batched YMM field addition for Curve25519. Each 256-bit \texttt{vpaddq} adds the same limb position across four independent field elements stored in structure-of-arrays layout.}


The limb-parallel example (\autoref{fig:simple-add}) demonstrates basic XMM support: 128-bit loads, lane-wise addition, and a 64-bit tail handled by \texttt{vmovq}. 
The batched example (\autoref{fig:batch-add}) exemplifies the full YMM pipeline: 256-bit loads and stores, memory source operands in \texttt{vpaddq}, and \texttt{vzeroupper} to avoid AVX/SSE transition \question{penalties}{This feels thin for a results section, should I include some qualitative observations or anything else?}.
...
Second, this work only verifies externally hand-written assembly. There is currently no way to automatically synthesize vectorized programs through Fiat Cryptography or CryptOpt.
