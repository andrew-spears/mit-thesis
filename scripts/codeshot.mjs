...
const sourceFile = resolve(repoRoot, 'MIT-thesis-template/body.tex');
const outDir = resolve(repoRoot, 'MIT-thesis-template/code-screenshots');
...
const highlighter = await createHighlighter({
  themes: [THEME],
  langs: [
    { ...coqGrammar, name: 'coq', scopeName: 'source.coq' },
    'asm',
  ],
});
...
await page.setViewport({ width: 2400, height: 200, deviceScaleFactor: 2 });
...
for (const { name, lang, code } of blocks) {
  ...
  const fullHtml = `<!doctype html><html><head><style>
    ...
    body { padding: 24px; display: inline-block; font-family: Menlo, monospace; }
    pre.shiki { margin: 0; padding: 16px 20px; border-radius: 8px; font-size: 14px; line-height: 1.55; }
